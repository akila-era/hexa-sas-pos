...
import { subscriptionsService } from '../services/superadmin.service';

const Subscription = () => {
  const [data, setData] = useState(subscription_details);
  const [rows, setRows] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);
  const [totalRecords, setTotalRecords] = useState(data.length);
  const [selectedSubscriptions, setSelectedSubscriptions] = useState([]);
  const [loading, setLoading] = useState(false);
  const [stats, setStats] = useState({ totalTransaction: 5340, totalSubscribers: 600, activeSubscribers: 560, expiredSubscribers: 40 });
  ...
  const fetchSubscriptions = useCallback(async () => {
    setLoading(true);
    try {
      const response = await subscriptionsService.getAll({ page: currentPage, limit: rows });
      if (response?.success && response?.data) {
        const subscriptionsData = response.data.subscriptions || response.data;
        // Transform data to match UI format
        const transformedData = subscriptionsData.map((sub, index) => ({
          key: sub.id || String(index + 1),
          id: sub.id,
          CompanyName: sub.company?.name || `Company ${index + 1}`,
          BillCycle: sub.billingCycle || 30,
          PaymentMethod: sub.paymentMethod || 'Credit Card',
          Email: sub.company?.email || `user${index}@example.com`,
          AccountURL: sub.company?.accountUrl || `company${index}.example.com`,
          Plan: sub.package?.name ? `${sub.package.name} (${sub.package.type || 'Monthly'})` : 'Basic (Monthly)',
          CreatedDate: new Date(sub.createdDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }),
          ExpiringDate: new Date(sub.expiringDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }),
          Image: `company-${(index % 10) + 1}.svg`,
          Status: sub.status || 'Paid',
          Amount: `$${sub.amount}`,
          InvoiceID: sub.invoiceId || `INV${String(index + 1).padStart(3, '0')}`
        }));
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      if (response?.success && response?.data) {
        setStats({
          totalTransaction: response.data.totalTransaction || 5340,
          totalSubscribers: response.data.totalSubscribers || 600,
          activeSubscribers: response.data.activeSubscribers || 560,
          expiredSubscribers: response.data.expiredSubscribers || 40
        });
      }
    } catch (error) {
  ...
  const handleDeleteSubscription = async () => {
    if (!deleteId) return;
    try {
      const response = await subscriptionsService.delete(deleteId);
      if (response?.success) {
        fetchSubscriptions();
        fetchStats();
      }
  ...
  },
  {
    ...
    <div className="action-icon d-inline-flex">
        ...
        onClick={() => handleViewSubscription(rowData)}>
                    ...
                    <div className="row align-items-center">
                      <div className="col-7">
                        <div>
                          ...
                          <h5>${stats.totalTransaction.toLocaleString()}</h5>
                        </div>
                      </div>
                      <div className="col-5">
                        <ReactApexChart
            ...
            <div className="modal-content">
              <div className="modal-body text-center">
                ...
                <div className="d-flex justify-content-center">
                  ...
                  <Link to="#" className="btn btn-primary" data-bs-dismiss="modal" onClick={handleDeleteSubscription}>
                    Yes, Delete
                  </Link>
                </div>
              </div>
