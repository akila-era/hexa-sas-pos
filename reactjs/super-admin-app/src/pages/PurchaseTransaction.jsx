...
import { transactionsService } from '../services/superadmin.service';

const PurchaseTransaction = () => {
  const [data, setData] = useState(purchase_transaction);
  ...
  const fetchTransactions = useCallback(async () => {
    setLoading(true);
    try {
      const response = await transactionsService.getAll({ page: currentPage, limit: rows });
      if (response?.success && response?.data) {
        const transactionsData = response.data.transactions || response.data;
        // Transform data to match UI format
        const transformedData = transactionsData.map((trans, index) => ({
          key: trans.id || String(index + 1),
          id: trans.id,
          CompanyName: trans.company?.name || `Company ${index + 1}`,
          Email: trans.company?.email || `user${index}@example.com`,
          PaymentMethod: trans.paymentMethod || 'Credit Card',
          CreatedDate: new Date(trans.createdDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }),
          Image: `company-${(index % 10) + 1}.svg`,
          Status: trans.status || 'Paid',
          Amount: `$${trans.amount}`,
          InvoiceID: trans.invoiceId || `INV${String(index + 1).padStart(3, '0')}`
        }));
