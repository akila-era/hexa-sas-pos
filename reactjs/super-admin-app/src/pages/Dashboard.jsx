...
import { dashboardService, domainsService } from "../services/superadmin.service";

const SuperAdminDashboard = () => {
  const routes = all_routes;
  const [stats, setStats] = useState({
    totalCompanies: 5468,
    activeCompanies: 4598,
    totalSubscribers: 3698,
    totalEarnings: 89878.58,
    companiesGrowth: '+19.01%',
    activeGrowth: '-12%',
    subscribersGrowth: '+6%',
    earningsGrowth: '-16%'
  });
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      if (statsResponse?.success && statsResponse?.data) {
        setStats(prev => ({
          ...prev,
          totalCompanies: statsResponse.data.totalCompanies || prev.totalCompanies,
          activeCompanies: statsResponse.data.activeCompanies || prev.activeCompanies,
          totalSubscribers: statsResponse.data.totalSubscribers || prev.totalSubscribers,
          totalEarnings: statsResponse.data.totalEarnings || prev.totalEarnings
        }));
      }

      // Fetch recent transactions
      const transResponse = await dashboardService.getRecentTransactions(5);
      if (transResponse?.success && transResponse?.data) {
        setRecentTransactions(transResponse.data);
      }

      // Fetch recent companies
      const companiesResponse = await dashboardService.getRecentCompanies(5);
      if (companiesResponse?.success && companiesResponse?.data) {
        setRecentCompanies(companiesResponse.data);
      }

      // Fetch expired plans
      const expiredResponse = await dashboardService.getExpiredPlans(5);
      if (expiredResponse?.success && expiredResponse?.data) {
        setExpiredPlans(expiredResponse.data);
      }

      // Fetch pending domains
      const pendingResponse = await dashboardService.getPendingDomains(5);
      if (pendingResponse?.success && pendingResponse?.data) {
        setPendingDomains(pendingResponse.data);
      }
    } catch (error) {
  ...
  const handleApproveDomain = async (id) => {
    try {
      const response = await domainsService.approve(id);
      if (response?.success) {
        fetchDashboardData();
      }
    } catch (error) {
  ...
  const handleRejectDomain = async (id) => {
    try {
      const response = await domainsService.reject(id);
      if (response?.success) {
        fetchDashboardData();
      }
    } catch (error) {
            ...
            <div className="card flex-fill">
              <div className="card-body">
                <div className="d-flex align-items-center justify-content-between">
                  ...
                  <span className={`badge ${stats.companiesGrowth.startsWith('+') ? 'bg-success' : 'bg-danger'} fw-normal mb-3`}>
                    {stats.companiesGrowth}
                  </span>
                </div>
                <div className="d-flex align-items-center justify-content-between">
                  <div>
                    <h2 className="mb-1">{stats.totalCompanies.toLocaleString()}</h2>
            ...
            <div className="card flex-fill">
              <div className="card-body">
                <div className="d-flex align-items-center justify-content-between">
                  ...
                  <span className={`badge ${stats.activeGrowth.startsWith('+') ? 'bg-success' : 'bg-danger'} fw-normal mb-3`}>
                    {stats.activeGrowth}
                  </span>
                </div>
                <div className="d-flex align-items-center justify-content-between">
                  <div>
                    <h2 className="mb-1">{stats.activeCompanies.toLocaleString()}</h2>
            ...
            <div className="card flex-fill">
              <div className="card-body">
                <div className="d-flex align-items-center justify-content-between">
                  ...
                  <span className={`badge ${stats.subscribersGrowth.startsWith('+') ? 'bg-success' : 'bg-danger'} fw-normal mb-3`}>
                    {stats.subscribersGrowth}
                  </span>
                </div>
                <div className="d-flex align-items-center justify-content-between">
                  <div>
                    <h2 className="mb-1">{stats.totalSubscribers.toLocaleString()}</h2>
            ...
            <div className="card flex-fill">
              <div className="card-body">
                <div className="d-flex align-items-center justify-content-between">
                  ...
                  <span className={`badge ${stats.earningsGrowth.startsWith('+') ? 'bg-success' : 'bg-danger'} fw-normal mb-3`}>
                    {stats.earningsGrowth}
                  </span>
                </div>
                <div className="d-flex align-items-center justify-content-between">
                  <div>
                    <h2 className="mb-1">${stats.totalEarnings.toLocaleString()}</h2>
