...
import { domainsService } from "../services/superadmin.service";

const Domain = () => {
  const [listData, setListData] = useState(domainData);
  ...
  const fetchDomains = useCallback(async () => {
    setLoading(true);
    try {
      const response = await domainsService.getAll({ page: currentPage, limit: rows });
      if (response?.success && response?.data) {
        const domainsData = response.data.domains || response.data;
        // Transform data to match UI format
        const transformedData = domainsData.map((domain, index) => ({
          id: domain.id || index + 1,
          img: company01,
          name: domain.company?.name || `Company ${index + 1}`,
          domainUrl: domain.domainUrl,
          plan: domain.plan || 'Basic (Monthly)',
          createdDate: new Date(domain.createdDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }),
  ...
  const handleDeleteDomain = async () => {
    if (!deleteId) return;
    try {
      const response = await domainsService.delete(deleteId);
      if (response?.success) {
        fetchDomains();
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      {/* /Domain Details */}
    <DeleteModal onDelete={handleDeleteDomain} />
    </>);
